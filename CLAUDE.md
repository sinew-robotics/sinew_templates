...
4. relevant `docs/conferences/<event>.md` when preparing for that delivery context
5. `docs/authoring.md`
6. `docs/figures-and-tables.md`
7. `docs/accessibility.md`
8. `deck.qmd`, exactly one selected `_quarto-color-*.yml` file, and included slides

Run `git status`, `quarto --version`, and `python3 scripts/validate.py` before substantial edits. Preserve user changes. Never create `.beads` in this repository; use an external planning workspace if the user requests Beads.
...
Color profiles own palette, fonts, surfaces, syntax highlighting, and chart tokens only. Preserve provenance and projection/accessibility changes in `docs/styles.md`.
...
- `deck.qmd` is the explicit order.
...
For theme/profile/core changes run `scripts/render-styles.sh`. Inspect representative outputs visually and offline. Verify the event's final PDF/video/upload artifact separately.
