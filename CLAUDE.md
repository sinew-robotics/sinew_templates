...
- Slide SOURCE uses only printable ASCII characters from a standard US keyboard. This is machine-checkable: permit ASCII 0x20-0x7E plus newline and tab, and reject every other character; `validate.py` enforces it. RENDERED visible text is narrower and different: Pandoc smart typography (authored `--` becoming an en dash, straight quotes becoming curly) and Quarto's cross-reference non-breaking spaces are permitted, but decorative Unicode -- arrows, math glyphs, emoji, box-drawing, symbols -- is not. Fix those at the source with words or ASCII equivalents. Do not chase pure ASCII in the render: the non-breaking spaces are load-bearing for cross-reference spacing, and disabling smart typography costs typography across every deck for no projection-safety gain.
...
- A slide that overflows its frame is a defect, not a style preference; render success and valid HTML do not mean content fit. Default budgets, not hard limits: 3 bullets per list, 10 words per bullet, 60 body words per slide, 5 table rows, 1-line captions, 2 nesting levels. `template/scripts/check_overflow.py` is a required gate (headless Chrome, viewport-invariant; exit 2 means it did not run, which is not a pass), but it catches escape, not crowding -- still inspect every slide visually.
- Quarto wraps every numbered `#fig-` object in an outer `div.quarto-float` before the real `<figure>`, so a layout container's `> figure` selector matches nothing -- style the wrapper, and pass layout down with subgrid rather than `display: contents`, which strips the figure's box and its inspector interactivity. A `:not(:has(...))` argument carrying an attribute selector outranks a bare class selector regardless of source order, so `figure:not(:has(img[style^="width"], video[style^="width"]))` silently beat a later, more specific-looking `figure:has(...) { width: 100% }` rule for any figure with no `<img>`/`<video>` at all, shrinking such figures to 75-84% of their column; confirm the winner with CDP `CSS.getMatchedStylesForNode`, never by source order. A `width: 100%` container with horizontal padding needs `box-sizing: border-box` or it overruns its parent by exactly the padding. Heading attributes (`# Title {.foo}`) land on the enclosing `<section>`, not the heading: `h1.foo` matches nothing, `.foo h1` works. `.metric strong { display: block }` puts bold text on its own line, so a `**bold**, trailing text` pattern inside `.metric` orphans the comma; keep metric values plain text. Put figure detail in `fig-alt`, not the `![...]` brackets -- a long bracket caption plus a separate `.figcap` produces two stacked captions since both share the same caption styling. An empty `[@]` citation key renders as literal visible bracket text with no warning; treat it as a defect.
...
Default: once artwork is transparent (stripped or native), give it `.plate`. Transparent art inherits whatever slide surface it lands on, and that risk is profile-dependent -- the repo's own demo asset measures roughly 1.1-1.2:1 contrast against `origami`/`blueprint` but 19.7:1 against `movement` -- so a later profile switch can silently make line art invisible. `.plate` also never alters source pixels, which matters for reused third-party figures. Omit it only as a deliberate opt-out, checked against the profile actually in use.
