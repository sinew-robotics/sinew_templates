...
- Added `template/scripts/check_overflow.py`, a headless-browser overflow gate that measures actual slide geometry and exits 2 (not 0) when it cannot run, and wired it into CI and both documented completion-gate sequences so an overflow regression cannot ship unnoticed.
