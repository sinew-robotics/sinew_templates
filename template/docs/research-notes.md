...
Last researched: **2026-08-25** (Asia/Seoul); the CSL and Reveal-source entries below were added and inspected on 2026-08-30, as was the third-party reference-figure provenance entry under "Third-party reference figures". The event-delivery pages under "Event delivery evidence" were not re-checked on 2026-08-30 and keep their original 2026-08-25 verification.
...
## Third-party reference figures

`template/assets/media/reference/vjepa2-abstract-new.png` (added 2026-08-30)
is a third-party image, not generated by this repository. It depicts V-JEPA 2
(Meta AI) and is included as the hard-case worked example for
`template/scripts/make_transparent.py`, shown alongside its transparent
derivative (`vjepa2-abstract-transparent.png`) in the deck's transparency
demo slides.

**Provenance status: unverified, included by owner decision.** Redistribution
permission for this file has not been established -- no license, usage
grant, or official source URL has been confirmed. Its inclusion in this
public, GitHub-Pages-deployed repository was an explicit decision by the
repository owner, made after the licensing question was raised (quoting
this project's own rule, "A citation is not permission to reuse a
copyrighted visual. Record license/permission or redraw within allowed
use."). That decision authorizes inclusion; it is not a verified license
and must not be read as one. This entry records that the decision was made
and that the permission question remains open, not that permission was
granted.
